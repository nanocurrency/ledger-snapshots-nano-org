...
# qbittorrent-nox WebUI/API. Bound to the tailnet address only (not
# loopback -- WebUI\Address is set to the tailnet IP explicitly, see
# snap-gni.9), so even though this script runs locally on the same box it
# must reach the API via the tailnet IP, not localhost. Resolved at
# runtime rather than hardcoded in case this ever runs on a different box.
QBIT_HOST = subprocess.run(
    ["tailscale", "ip", "-4"], check=True, capture_output=True, text=True
).stdout.strip()
QBIT_API_BASE = f"http://{QBIT_HOST}:8080"

# Credentials are intentionally NOT hardcoded here -- read from env vars
# set in a restricted-permission file (e.g. /etc/nanocurrency/qbittorrent.env,
# 0600 root:root) sourced by whatever runs this script (cron/systemd timer),
...
def check_prerequisites():
    ...
    # run.
    required_tools = [node_path, "7z", "mktorrent", "git", "tailscale", "sudo"]
    missing = []
...
def seed_torrent(torrent_path):
    ...
    login = session.post(f"{QBIT_API_BASE}/api/v2/auth/login",
                          data={"username": QBIT_USERNAME, "password": QBIT_PASSWORD})
    if login.text.strip() != "Ok.":
        raise RuntimeError(f"qbittorrent-nox login failed: {login.status_code} {login.text}")
